...
function postUnusualApi(json) {
  ...
  // 2.1.12	员工在岗情况整体占比图
  api('/mobilityAnalysis/empPostOverallSituation', 3001,
      'cbEmpPostOverallSituation', JSON.stringify(apiReq));

}
function postAbnormalRanking(postAbnormalDuration) {
  if (postAbnormalDuration.indexOf('>') > -1) {
    postAbnormalDuration = postAbnormalDuration.split('>')[1].split('<')[0];
  }
  // 在岗异常时长	postAbnormalDuration	必填	String
  apiReq.postAbnormalDuration = postAbnormalDuration.replace('小时', '').trim();
  // 2.1.13	在岗异常情况排名表
  api('/mobilityAnalysis/postAbnormalRanking', 3001, 'cbPostAbnormalRanking',
...
function formDataDay(day) {
  postUnusualClick(getItem("getArea"), getItem("getArea1"), getItem("getPost"),
      day);
}
