...
function cbPositionStabilityIndex(json) {
  ...
  if (checkCallBackJsonIsNotNull(json) && checkJsonDateIsNotNull(json.data)) {
    $.each(json.data, function (i, item) {
      ...
      scaleMap2Stage('post_change_twig', 'norm-ltwig', 'norm-rtwig', item.index,
          getRProportion(item.index, 0));
    });
    empInPostIndex();
  }
...
function cbEmpInPostIndex(json) {
  ...
  if (checkCallBackJsonIsNotNull(json) && checkJsonDateIsNotNull(json.data)) {
    $.each(json.data, function (i, item) {
      ...
      scaleMap2Stage('post_unusual_twig', 'norm-ltwig', 'norm-rtwig',
          item.index, getRProportion(item.index, 0));
    });
...
function cbTurnoverIntentionIndex(json) {
  ...
  if (checkCallBackJsonIsNotNull(json) && checkJsonDateIsNotNull(json.data)) {
    $.each(json.data, function (i, item) {
      ...
      scaleMap2Stage('leave_twig', 'norm-ltwig', 'norm-rtwig', item.index,
          getRProportion(item.index, 0));
    });
    remainInCity();
  }
...
function cbRemainInCity(json) {
  ...
  if (checkCallBackJsonIsNotNull(json) && checkJsonDateIsNotNull(json.data)) {
    $.each(json.data, function (i, item) {
      ...
      checkIndexRedByIndex(item.index, 'remain_in_city', warningValueNum);
      // scaleMap2('leave_direction_twig', 'norm-ltwig', 'norm-rtwig', item.index, getRProportion(item.index, 0));
      scaleMap2Stage('leave_direction_twig', 'norm-ltwig', 'norm-rtwig',
          item.index, getRProportion(item.index, 0));
    });
  }
}